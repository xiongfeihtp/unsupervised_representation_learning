...
def batch_arg_scope(weight_decay=0.00004,
                    stddev=0.1,
...
class Auto_Encoder_Model(object):
    ...
    def encoder(self):
        with slim.arg_scope(batch_arg_scope(is_training=self.is_training())):
            with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, padding='SAME'):
                with tf.variable_scope('encoder'):
                    ...
                    conv1 = slim.max_pool2d(conv1, [2, 2], stride=2, scope='encoder_maxpool1',
                                            padding='SAME')  # [112, 112, 64]

                    conv2 = slim.conv2d(conv1, 128, [3, 3], scope='encoder_conv2')
                    conv2 = slim.max_pool2d(conv2, [2, 2], stride=2, scope='encoder_maxpool2',
                                            padding='SAME')  # [56, 56, 128]

                    conv3 = slim.conv2d(conv2, 256, [3, 3], scope='encoder_conv3')
                    self.conv3 = slim.max_pool2d(conv3, [2, 2], stride=2, scope='encoder_maxpool3',
                                                 padding='SAME')  # [28, 28, 256]
                    self.feature = tf.reshape(self.conv3, [self.config.batch_size, -1])

    def decoder(self):
        with slim.arg_scope(batch_arg_scope(is_training=self.is_training())):
            with slim.arg_scope([slim.conv2d], kernel_size=[3, 3], padding='SAME', activation_fn=tf.nn.relu):
                with tf.variable_scope('decoder'):
                    conv4 = tf.image.resize_nearest_neighbor(self.conv3, (56, 56))
                    conv4 = slim.conv2d(conv4, 128, scope='decoder_conv1')  # [56, 56, 128]

                    conv5 = tf.image.resize_nearest_neighbor(conv4, (112, 112))
                    conv5 = slim.conv2d(conv5, 64, scope='decoder_conv2')  # [112, 112, 64]

                    conv6 = tf.image.resize_nearest_neighbor(conv5, (224, 224))
                    self.conv6 = slim.conv2d(conv6, 32, scope='decoder_conv3')  # [224, 224, 32]

    def build_model(self):
        logits_ = slim.conv2d(self.conv6, 3, [3, 3], padding='SAME', activation_fn=None)  # [224, 224, 3]
        self.outputs_ = tf.nn.sigmoid(logits_, name='output_')
        tf.summary.image("recontructed_images", self.outputs_)
